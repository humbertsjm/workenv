...
from tests.utils import (
    get_random_string_from_length,
    get_random_activity_status_option,
    get_random_activity_type_option
)
import random
...
class TestActivitiyForms():
    ...
    def test_non_int_ask_status(self, monkeypatch: MonkeyPatch) -> None:
        random_str = get_random_string_from_length(5)
        monkeypatch.setattr(
            'builtins.input',
            lambda _: random_str
    ...
    def test_invalid_option_ask_status(self, monkeypatch: MonkeyPatch) -> None:
        random_option: set[int] = set(list(range(len(ActivityStatus) + 5)))
        random_option = random_option.difference([s.value for s in ActivityStatus])
        monkeypatch.setattr(
            'builtins.input',
            lambda _: str(random.choice(list(random_option)))
        )
        with pytest.raises(InvalidActivityStatusOptionError):
            ActivityForm.ask_status_input()

    def test_ask_type(self, monkeypatch: MonkeyPatch) -> None:
        random_type: int = get_random_activity_type_option().value
        monkeypatch.setattr(
            'builtins.input',
            lambda _: str(random_type)
        )
        asked_input: int = ActivityForm.ask_type_input()
        assert isinstance(asked_input, int)
        assert asked_input == random_type

    def test_non_int_ask_type(self, monkeypatch: MonkeyPatch) -> None:
        random_str = get_random_string_from_length(5)
        monkeypatch.setattr(
            'builtins.input',
            lambda _: random_str
        ...
        with pytest.raises(ValueError):
            ActivityForm.ask_type_input()

    def test_invalid_option_ask_type(self, monkeypatch: MonkeyPatch) -> None:
        random_option: set[int] = set(list(range(len(ActivityType) + 5)))
        random_option = random_option.difference([s.value for s in ActivityType])
        monkeypatch.setattr(
            'builtins.input',
            lambda _: str(random.choice(list(random_option)))
        )
        with pytest.raises(InvalidActivityTypeOptionError):
            ActivityForm.ask_type_input()
